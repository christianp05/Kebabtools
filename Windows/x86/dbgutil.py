...
def GetContext(thread):
    ...
    result = kernel32.Wow64GetThreadContext(thread, byref(ctx))
    if result == 0:
        raise ctypes.WinError(ctypes.get_last_error())
    ResumeThread(thread)
    return ctx
...
def SetContext(thread, ctx):
    SuspendThread(thread)
    result = kernel32.Wow64SetThreadContext(thread, ctx)
    if result == 0:
        raise ctypes.WinError(ctypes.get_last_error())
    ResumeThread(thread)
...
def ContinueFromBreak(process):
    ...
    neweip = process.GetRegs().eip - 1
    process.SetReg("eip", neweip)
    process.SingleStep()
